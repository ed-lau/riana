"""
M3 Week 0 stub: baseline-subtraction integration comparison.
...
chromatographic baseline / background is handled.

WEEK 0 STATUS — STUB. v0.9.0 does no baseline subtraction at all: integration
is a rectangle over a fixed RT window, so baseline is included in proportion
to RT width (PROJECT_REVIEW.md §2c point 2). This script currently records
the one-method `none` baseline. It is committed now, before the rewrite, so
Week 3 has an unchanged gate.

WEEK 3 — register the baseline-subtracted integration runs as extra
`--method` entries:
  none       v0.9.0 — no subtraction
  linear     local linear between detected boundaries (Skyline default)
  snip       SNIP via pybaselines
  asls       asymmetric least squares via pybaselines
The comparison table then shows whether baseline subtraction moves m0_rmse.
Per M2 finding 2, watch the *uncurated* population row — low-SNR peptides,
where baseline is the largest relative contaminant, are exactly the ones the
R²>0.95 gate discards.
...
Usage:
  python bench_baseline.py \
    --method none=<integrate_dir> [--method linear=<dir> ...] \
    --ground-truth <ground_truth.csv> \
    --coefficients <d2o_aa_coefficients_<line>.csv> \
    --output-dir <dir>

Outputs (written under --output-dir):
  - baseline_comparison.csv   (method x population: m0/env RMSE)
  - baseline_comparison.json  (per-method per-proportion detail)
"""
...
def main() -> None:
    ...
    if len(methods) == 1:
        print('[stub] one method only — recording the no-baseline baseline. '
              'Week 3 adds linear / snip / asls methods.')
    ...
    comparison.to_csv(args.output_dir / 'baseline_comparison.csv', index=False)

    out = {
        'benchmark': 'baseline',
        'status': 'week0_stub_no_baseline_only' if len(methods) == 1
        else 'multi_method',
    ...
    with (args.output_dir / 'baseline_comparison.json').open('w') as f:
